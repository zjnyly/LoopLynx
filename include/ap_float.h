...
public:
...
#else
    bits = (const bits_t &)other; // TODO PLACEHOLDER
#endif
  }
  ...
  explicit operator ap_fixed<W2, I2, Q2, O2, N2>() const noexcept {
    ...
    return res;
#else
    return *((const ap_float<W, E> *)this); // TODO PLACEHOLDER
#endif
  }
...
std::istream &operator>>(std::istream &os, ap_float<W, E> &val) noexcept {
  double rd;
  os >> rd;
  val.from_double(rd); // FIXME Raw double conversion may do weird truncation
  return os;
}
#endif
...
struct numeric_limits<ap_float<W, E>> {
  ...
  static constexpr int digits10 = floor((digits - 1) * log10(2));
  static constexpr int max_digits10 = ceil(digits * log10(2) + 1);

  static constexpr int radix = 2;
  static constexpr int min_exponent = (-1 << (E - 1)) + 3;
  ...
  static constexpr ap_float<W, E> denorm_min() noexcept {
    return ap_float<W, E>::denorm_min();
  }
